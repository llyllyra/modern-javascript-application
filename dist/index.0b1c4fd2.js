...
window.addEventListener("load", ()=>{
    (0, _fetchWeatherJsDefault.default)("namur");
});
...
form.addEventListener("keypress", (e)=>{
    if (e.key === "Enter") {
        (0, _fetchWeatherJsDefault.default)(City);
        e.preventDefault();
        input.value = "";
    }
});
...
var _weatherDisplayJs = require("./weatherDisplay.js");
var _weatherDisplayJsDefault = parcelHelpers.interopDefault(_weatherDisplayJs);
var _axios = require("axios");
var _axiosDefault = parcelHelpers.interopDefault(_axios);
let dataWeather = [];
const fetchWeather = async (city)=>{
    let url = "https://api.openweathermap.org/data/2.5/forecast/daily?q=" + city + "&units=metric&lang=fr&cnt=7&appid=" + (0, _configJsDefault.default).key;
    await (0, _axiosDefault.default).get(url).then((res)=>{
        dataWeather = res.data;
    });
    (0, _weatherDisplayJsDefault.default)(dataWeather);
};
...
var _chartsJsDefault = parcelHelpers.interopDefault(_chartsJs);
let labels = [];
let data = [];
const weatherDisplay = (dataWeather)=>{
    ...
    result.innerHTML = `
        ...
        `;
    today.innerHTML = `
        <h4>${(0, _getDateJs.formatToLocalTime)(dataWeather.list[0].dt)}</h4>
        <section class="text">
        <img 
    ...
    labels.push((0, _getDateJs.AddDay)(dataWeather.list[0].dt));
    for(let i = 1; i < 6; i++){
        ...
        day.innerHTML = `
        <h4>${(0, _getDateJs.formatToLocalTime)(dataWeather.list[i].dt)}</h4>
    ...
    (0, _chartsJsDefault.default)(labels, data);
};
...
const formatToLocalTime = (secs, format = "cccc, dd LLL yyyy")=>(0, _luxon.DateTime).fromSeconds(secs).toFormat(format);
const AddDay = (secs)=>(0, _luxon.DateTime).fromSeconds(secs).weekdayShort;

},{"luxon":"dpK6X","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"dpK6X":[function(require,module,exports) {
"use strict";
